...
<html>
    <head>
        ...
        <!--Codice Statistiche Google Analytics Iury Mazzoni ## NON CANCELLARE!! ## -->
        <%if cat>0 then%>
        <link rel="canonical" href="http://www.cristalensi.it/prodotti.asp?cat=<%=cat%>"/>
        <%end if%>
        <%if FkProduttore>0 then%>
        <link rel="canonical" href="http://www.cristalensi.it/prodotti.asp?FkProduttore=<%=FkProduttore%>"/>
        <%end if%>
    </head>
    <body>
        <div id="wrap">
								...
								%>
                                    ...
                                    <li class="clearfix">
                                        ...
                                        <div class="data">
                                            ...
                                            <p><%=Left(descrizione_prodotto,150)%><%if Len(descrizione_prodotto)>150 then%>...</p><%end if%>
                                            <%if FkCategoria2>0 then%><span style="display:block;margin-bottom: 10px;">Categoria: <a href="prodotti.asp?cat=<%=FkCategoria2%>" class="button_link scheda-link" title="Elenco prodotti della stessa categoria: <%=titolo_cat%>"><%=titolo_cat%></a></span><%end if%>
                                            <%if fkproduttore_pr>0 then%><span class="produttore" style="display:block;margin-bottom: 10px;">Produttore: &nbsp; <a href="prodotti.asp?FkProduttore=<%=fkproduttore_pr%>" class="button_link scheda-link" title="Elenco prodotti dello stesso produttore: <%=produttore%>"><strong><%=produttore%></strong></a></span><%end if%>
							 ...
							 %>
								...
								%>
                                    <li class="clearfix">
                                        ...
                                        <div class="data">
                                            <a href="<%=NomePagina%>" title="<%=titolo_prodotto%>&nbsp;<%=codicearticolo%> - <%=titolo_cat%>"><strong><%=titolo_prodotto%></strong><%if codicearticolo<>"" then%>&nbsp;[<%=codicearticolo%>]<%end if%></a>
                                            <p><%=Left(descrizione_prodotto,150)%><%if Len(descrizione_prodotto)>150 then%>...<%end if%></p>
                                                <%if FkCategoria2>0 then%><span style="display:block;margin-bottom: 10px;"><small>Categoria:</small> <a href="prodotti.asp?cat=<%=FkCategoria2%>" class="button_link scheda-link" title="Elenco prodotti della stessa categoria: <%=titolo_cat%>"><%=titolo_cat%></a></span><%end if%>
                                                    <%if fkproduttore>0 then%><span class="produttore" style="display:block;margin-bottom: 10px;"><small>Produttore:</small> <a href="/prodotti.asp?FkProduttore=<%=fkproduttore%>" class="button_link scheda-link" title="Elenco prodotti dello stesso produttore: <%=produttore%>"><strong><%=produttore%></strong></a></span><%end if%>
                                            <%if prezzoarticolo=0 then%>
                                            <p class="cart clearfix"><span class="price">Prezzo listino: <span><%=prezzolistino%>&#8364;</span></span>&nbsp;&nbsp;<a href="#" onClick="MM_openBrWindow('/richiesta_informazioni.asp?codice=<%=codicearticolo%>&titolo=<%=titolo_prodotto%>&amp;produttore=<%=produttore%>&amp;id=<%=id%>','','width=650,height=650,scrollbars=yes')" class="cart-link button_link_red">Prezzo Cristalensi? clicca qui per un preventivo dal nostro staff</a></p>
                                            <%else%>
                                                <p class="cart clearfix"><%if prezzolistino<>0 then%><span class="price">Prezzo listino: <span><%=prezzolistino%>&#8364;</span></span><%end if%>&nbsp;&nbsp;<%if prezzoarticolo<>"" then%><span class="cristalprice">Prezzo Cristalensi: <%=prezzoarticolo%>&#8364;</span>&nbsp;&nbsp;<small><i>Iva compresa</i></small><%end if%><a href="<%=NomePagina%>" title="Inserisci&nbsp;nel&nbsp;carrello&nbsp;<%=titolo_prodotto%>&nbsp;<%=codicearticolo%>" class="button_link right scheda-link button_link_red"><span>Scheda prodotto</span></a></p>
                                            <%end if%>
                                        </div>
                                    </li>
                                    <% 
