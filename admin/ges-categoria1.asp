          ...
          <!--tab centrale-->
			<table width="98%"  border="0" cellspacing="0" cellpadding="0">
              ...
              <tr class="admin-intestazione" align="left"> 
                <td width="64%">&nbsp;<a href="ges-categoria1.asp?ordine=0">Cod.</a>&nbsp;Categoria&nbsp;<a href="ges-categoria1.asp?ordine=1">A/Z</a>&nbsp;<a href="ges-categoria1.asp?ordine=2">Z/A</a></td>
                <td width="28%">Posizione</td>
              ...
              <tr align="left" class="admin-righe" <% if t = 1 then %>bgcolor="#CFCFCF"<% end if %>> 
                <td>&nbsp;<a href="sche-categoria1.asp?pkid=<%=nrs("pkid")%>&ordine=<%=ordine%>"><font color="#CC0000"><%=nrs("pkid")%>.</font><%=nrs("titolo")%></a></td>
                <td><%=nrs("posizione")%></td>
                <td align="center"><a href="sche-categoria1.asp?mode=1&pkid=<%=nrs("pkid")%>&C1=ON&ordine=<%=ordine%>&p=<%=p%>"><font color="#CC0000">X</font></a></td>
               </tr>
              ...
              <%else%>
              <tr> 
                <td colspan="3">Nessuna categoria presente</td>
              </tr>
              <%end if%>
              <tr> 
                <td colspan="3">&nbsp;</td>
              ...
              <tr class="admin-intestazione" align="left"> 
                <td colspan="3">&nbsp; <% if nrs.recordcount > 0 then %>
                  Pag. <strong><%=p%></strong> di <%=nrs.PageCount%> Vai alla pagina&nbsp; <% if p > 5 then %>
                  [<a href="ges-categoria1.asp?p=<%=p-5%>&ordine=<%=ordine%>">&lt;&lt; 
                  5 prec</a>] 
                  <% end if %> <% if p > 1 then %>
                  [<a href="ges-categoria1.asp?p=<%=p-1%>&ordine=<%=ordine%>">&lt; 
                  prec</a>] 
                  <% end if %> <% for page = p to p+4 %> <a href="ges-categoria1.asp?p=<%=Page%>&ordine=<%=ordine%>" class="testo"><%=page%></a> <% if page = nrs.PageCount then
		   		 		page = p+4
   		 				end if
	    				%> <% next %> <% if page-1 < nrs.PageCount then %>
                  [<a href="ges-categoria1.asp?p=<%=p+1%>&ordine=<%=ordine%>">succ 
                  &gt;</a>] 
                  <% end if %> <% if nrs.PageCount-page > 5 then %>
                  [<a href="ges-categoria1.asp?p=<%=p+5%>&ordine=<%=ordine%>">5 
                  succ &gt;&gt;</a>] 
                  <% end if%> &nbsp; &nbsp;[<a href="ges-categoria1.asp?p=<%=nrs.PageCount%>&ordine=<%=ordine%>">ultima 
                  pagina</a>] <% end if %> </td>
              </tr>
              <tr> 
                <td colspan="3">&nbsp;</td>
